...
export function FiltroDesktop({ icon = "", title = "", departamento = false, array = null, array2 = null, title2= null}) {
  ...
  const changeRadiosFilter = () => {
    setArrayFilter(tableSetores.filter((setor) => setor.departamentoId === filterDepartamento.current))
  }
    ...
    <div>
      ...
      {array &&
        array.map((valor) => {
          ...
          return (
            <>
              <input type="radio" id={valor.id} name={title} value={valor.id} key={valor.id} onChange={handleInputChangeDepartamento}></input>
      ...
      {departamento && arrayFilter.map((setor) => {
        ...
        return (
          <>
            <input type="radio" id={setor.id} name={"setor"} value={setor.id} key={setor.id} onChange={handleInputChangeSetor}></input>
      ...
      {array2 && array2.map((valor) => {

        return (
          <>
            <input type="radio" id={valor.id} name={title2} value={valor.id} key={valor.id} onChange={handleInputChangeCargo}></input>
            <label htmlFor={valor.id}>{valor.nome}</label><br></br>
          </>
        );
      })}
